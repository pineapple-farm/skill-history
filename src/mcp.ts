...
export function createMcpHandler(db: D1Database) {
  ...
  server.tool(
    ...
    async ({ handle, slug }) => {
      ...
      if (!skill) {
        return { content: [{ type: "text" as const, text: JSON.stringify({ error: "Skill not found", handle, slug }) }] };
      }

      const snapshots = await db
        .prepare("SELECT captured_at, downloads, installs_all_time FROM snapshots WHERE skill_id = ? ORDER BY captured_at ASC")
        .bind(skill.id)
        .all<{ captured_at: string; downloads: number; installs_all_time: number }>();

      return {
        content: [{
          type: "text" as const,
          text: JSON.stringify({ skill, snapshots: snapshots.results ?? [] })
        }]
      };
    }
  );
